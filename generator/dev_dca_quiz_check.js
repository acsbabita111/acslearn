/* ============================================================
   dev_dca_quiz_check.js — DCA-2036 अभ्यास-प्रश्न भंडार का check-robot
   v1.0 · 16-Aug-2026 · चलाना: node generator/dev_dca_quiz_check.js
   (dev_msh_quiz_check v1.0 का DCA-रूप — 22 खंड, /assets/dca_quiz/)
...
const PARTS = DCA_COURSE.parts.map(p => [p.from, p.to]);
let fails = 0, totalQ = 0, shards = 0;
function bad(msg){ console.log("  ❌ " + msg); fails++; }
const seenAll = {};
for (let k = 1; k <= PARTS.length; k++) {
  const kk = String(k).padStart(2, "0");
  const f = path.join(__dirname, "..", "assets", "dca_quiz", "dca_quiz_k" + kk + ".js");
  if (!fs.existsSync(f)) continue;
  shards++;
  const bank = require(f).DCA_QUIZ;
  const [a, b] = PARTS[k - 1];
