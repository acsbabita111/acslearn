/* ============================================================
   dev_ecom_quiz_check.js — ACS ई-कॉमर्स मास्टरी अभ्यास-प्रश्न भंडार का check-robot
   v1.0 · 18-Aug-2026 · चलाना: node generator/dev_ecom_quiz_check.js
   (dev_dca_quiz_check v1.0 का ecom-रूप — 24 खंड, /assets/ecom_quiz/)
...
const PARTS = ECOM_COURSE.parts.map(p => [p.from, p.to]);
...
for (let k = 1; k <= PARTS.length; k++) {
  ...
  const f = path.join(__dirname, "..", "assets", "ecom_quiz", "ecom_quiz_k" + kk + ".js");
  if (!fs.existsSync(f)) { warn("shard k" + kk + " फ़ाइल नहीं"); continue; }
  shards++;
  const bank = require(f).ECOM_QUIZ;
  const [a, b] = PARTS[k - 1];
  ...
  for (let n = a; n <= b; n++) {
    ...
    list.forEach((it, qi) => {
      ...
      if (/\becm\b/i.test(all)) bad(tag + ": कूट-नाम");
