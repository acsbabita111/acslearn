# -*- coding: utf-8 -*-
"""ecom_quiz_build.py — ACS ई-कॉमर्स मास्टरी: स्रोत (प्रश्न,सही,ग़लत×3) → assets/ecom_quiz/ecom_quiz_kNN.js
   ...
   सिर्फ़ 15-35% खिड़की में — जाँच dev_ecom_quiz_check.js से।
   चलाना (repo-रूट से): python3 generator/quiz_src/ecom_quiz_build.py            # सब 24 खंड
                        python3 generator/quiz_src/ecom_quiz_build.py 21         # एक खंड
"""
import json, sys, os, importlib.util, re
HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.abspath(os.path.join(HERE, "..", ".."))
OUTDIR = os.path.join(ROOT, "assets", "ecom_quiz")

def parts():
    src = open(os.path.join(ROOT, "generator", "data", "ecom_course_data.js"), encoding="utf-8").read()
    m = re.search(r"parts:\s*(\[[\s\S]*?\])\s*\n\};", src)
    return [(p["no"], p["from"], p["to"]) for p in json.loads(m.group(1))]
...
def build(k, a, b):
    srcp = os.path.join(HERE, "ecom_quiz_k%02d_src.py" % k)
    ...
    if fails: print("\n".join(fails))
    kk = "%02d" % k
    js = ('/* ecom_quiz_k%s.js — ACS ई-कॉमर्स मास्टरी अभ्यास-प्रश्न · खंड-%d (पाठ %d-%d)\n'
          ...
          '   स्रोत: generator/quiz_src/ecom_quiz_k%s_src.py → builder ecom_quiz_build.py (हाथ से न बदलें)\n'
          '   जाँच: node generator/dev_ecom_quiz_check.js · गिनती मशीन से देखें */\n'
          '"use strict";\nvar ECOM_QUIZ_K%s = %s;\n'
          'if (typeof module !== "undefined") module.exports = { ECOM_QUIZ: ECOM_QUIZ_K%s };\n') % (kk, k, a, b, kk, kk, json.dumps(bank, ensure_ascii=False), kk)
    os.makedirs(OUTDIR, exist_ok=True)
    open(os.path.join(OUTDIR, "ecom_quiz_k%s.js" % kk), "w", encoding="utf-8").write(js)
    return len(bank)
