...
const CACHE_VERSION = 'v312'; // (31-Jul) SERVER_EXAM_COURSES में SE009:true जोड़ा — ई-कॉमर्स भी अब 2400-प्रश्न server-मोड परीक्षा (ecom_bank.js) पर; dashboard.js का यही एकमात्र बदलाव, बाक़ी byte-अछूता। // EXAM ENGINE (30-Jul, Founder "अभी शुरू करो"): dashboard.js v6.0 adds server-attempt exam mode — one-question-at-a-time UI (examStartServer/examRenderServer/examSaveCurrent), calls new startCourseExam callable to draw 120 random Qs (shuffled options, correct answer never sent to client) from private functions/dca_bank.js (3600-Q merged bank, never on GitHub); submitCourseExam now branches: attemptId present => grades via examAttempts doc (qids+perms), else falls back byte-identical to old static-bank path (SE009 etc untouched); firestore.rules v12 adds examAttempts (owner/Founder/Admin read, write:false); PJ016 static exam_data.js entry kept only for gating metadata (minLessons/name/pass) — its old q[] array of 120 no longer read by client for PJ016. Runtime-harness-verified: 120 unique Qs, options genuinely shuffled, correct/wrong/missing-answer scoring all exact, prev/next navigation preserves picks, incomplete-answer detection works.
